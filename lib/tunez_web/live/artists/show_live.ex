defmodule TunezWeb.Artists.ShowLive do
    ...
    <div id={"album-#{@album.id}"} class="md:flex gap-8 group">
      ...
      <div class="flex-1">
        <.header class="pl-3 pr-2 !m-0">
          ...
          <:action :if={Tunez.Music.can_delete_album?(@current_user, @album)}>
            <.button_link
              size="sm"
              inverse
              kind="error"
  ...
  def handle_event("destroy-artist", _params, socket) do
    case Tunez.Music.delete_artist(socket.assigns.artist, actor: socket.assigns.current_user) do
      :ok ->
        socket =
          socket
      ...
      {:error, error} ->
        Logger.error("Failed to delete artist: #{socket.assigns.artist.id}: #{inspect(error)}")

        socket =
          put_flash(socket, :error, "Failed to delete artist.")

        {:noreply, socket}
    end
  end

  def handle_event("destroy-album", %{"id" => album_id} = _params, socket) do
    case Tunez.Music.delete_album(album_id, actor: socket.assigns.current_user) do
      :ok ->
        socket =
          socket
      ...
      {:error, error} ->
        Logger.error("Failed to delete album: #{album_id}: #{inspect(error)}")

        socket =
          put_flash(socket, :error, "Failed to delete album.")
