defmodule TunezWeb.Albums.FormLive do
  ...
  def mount(%{"id" => album_id} = _params, _session, socket) do
  ...
  def render(assigns) do
    ...
    <Layouts.app {assigns}>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input name="artist_id" label="Artist" value={@artist.name} />
  ...
  def handle_event("save", %{"form" => form_data}, socket) do
    case AshPhoenix.Form.submit(socket.assigns.form, params: form_data) do
      {:ok, album} ->
        socket =
          put_flash(socket, :info, "Album saved")
          |> push_navigate(to: ~p"/artists/#{album.artist_id}")

        {:noreply, socket}

      {:error, form} ->
        Logger.error("could not save album #{inspect(form)}")
        socket = put_flash(socket, :error, "Could not save album") |> assign(:form, form)
        {:noreply, socket}
    end
  end
